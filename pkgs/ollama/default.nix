...
#
# Adapted from nixpkgs pkgs/by-name/ol/ollama/package.nix
# with nixpkgs-internal test infrastructure removed.
#
# To update:
...
#   nix build .#ollama            # CPU only
#   nix build .#ollama-rocm       # AMD ROCm
#   nix build .#ollama-cuda       # NVIDIA CUDA
#   nix build .#ollama-vulkan     # Generic Vulkan
{
  lib,
  buildGoModule,
  fetchFromGitHub,
...
goBuild (finalAttrs: {
  ...
  ++ lib.optionals enableVulkan [ ccache ];
  ...
  postFixup = lib.optionalString (enableRocm || enableCuda) ''
  ...
  checkFlags =
    let
      skippedTests = [
        ...
        # Tries to install tools via npm, needs /bin/cat and /bin/chmod (sandbox)
